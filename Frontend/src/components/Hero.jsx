...
export default function Hero() {
  ...
  //image
  const HomeBGImage =
    "https://res.cloudinary.com/ddkgrqekv/image/upload/hero_section_image_r9cbsq.png";
  ...
  return (
    ...
    >
      <div
        className="relative w-full aspect-[20/9] min-h-[450px] flex items-center justify-start rounded-3xl overflow-hidden"
        style={{ boxShadow: "0px 10px 30px rgba(143, 169, 143, 0.25)" }}
      >
        {/* FIXED: Changed to object-top so the 'NUTRICARE' background branding is always visible and never cut off at the top */}
        <img
          src={HomeBGImage}
          alt="Home Page Background"
          className="hero-bg absolute inset-0 w-full h-full object-cover object-top z-0"
        />

        {/* Gradient overlay — strong dark on left where text lives, fades to transparent right */}
        <div className="absolute inset-0 z-[1] bg-gradient-to-r from-black/60 via-black/30 md:via-black/10 to-transparent" />
        ...
        {/* Overlay Content */}
        <div className="relative z-[2] flex flex-col items-start px-5 sm:px-10 md:px-16 lg:px-24 xl:px-32 w-full max-w-[95%] sm:max-w-[85%] md:max-w-[700px] lg:max-w-[800px] xl:max-w-[1000px] py-6 sm:py-0">
          <h1
